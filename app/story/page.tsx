  ...
  return (
    <ViewTransition enter="page-in" exit="page-out" default="none">
                ...
                >
                  <div className="flex items-start gap-5">
                    <MoonMark
                      phase={t.phase}
                      size={28}
                      className="mt-3 shrink-0 text-ink lg:mt-6"
                    />
                    <p
                      className={
                        isYear
      ...
      */}
          ...
          <div className="mt-10 grid gap-10 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,1fr)] lg:gap-20">
            <div className="space-y-5 text-ink-soft">
              <p className="max-w-prose">
                ...
                어르신들이 지속적으로 경제활동에 참여할 수 있는 환경을
                만들어가고 있습니다.
              </p>
              <p className="max-w-prose">
                또한 기업의 성장이 지역사회와 함께 이어질 수 있도록 매출의
                일부를 지역에 환원하며 선순환 구조를 만들어가고자 합니다.
              </p>
            </div>
            ...
            <ul className="space-y-6 border-t border-ink/15 pt-7 lg:border-l lg:border-t-0 lg:pl-14 lg:pt-1">
              {[
                {
                  year: "2020",
                  body: "장애인복지관·나주시다문화가족센터 등 사회복지시설 업무협약",
                },
                { year: "2021", body: "전라남도지사 표창 (사회복지부문)" },
                {
                  year: "2023",
                  body: "나주시 여성새로일하기센터·국립나주숲체원 업무협약",
                },
              ].map((r) => (
                <li key={r.year}>
      ...
      {/* 연혁 — 인증·표창·업무협약까지 사실로 뒷받침하는 자리. 한 섹션에 넷을 쌓지 않는다. */}
      <section className="rise border-t border-ink/10">
        <div className="section-y mx-auto max-w-6xl px-5 lg:px-8">
          <SectionEyebrow phase={1}>연혁</SectionEyebrow>
          <h2 className="mt-3 text-h2 lg:text-h2-lg">
