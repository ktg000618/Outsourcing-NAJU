...
- **소식 올리기**: `/admin/login` 에서 직원 계정으로 로그인 → 「새 글」. 제목·본문·날짜·사진(3장, 장당 5MB, 가로 사진 권장)·링크.
  ...
  사진은 게시 중이면 공개 버킷(`news`), 숨긴 글이면 비공개 버킷(`news-hidden`)에 있고 숨기기·게시가 옮깁니다.
  올리고 저장하지 않은 사진은 다음 저장 때 하루 지난 것만 자동으로 지워집니다 — 손으로 정리할 일은 없습니다.
...
DB 스키마·RLS·스토리지 버킷은 `supabase/migrations/` 의 파일들(소식 → 예약 문의 → 공지 띠 → 직원 허용 목록·90일 보존 → 원장 보정 → 비공개 사진 버킷)이 전부이고, 프로덕션(gjunovajgvyxhbnicznf)에는 모두 적용돼 있습니다. 새 변경은 `supabase migration new <이름>` 으로 파일을 만들고 `supabase login` → `supabase link --project-ref gjunovajgvyxhbnicznf` → `supabase db push` 로 적용합니다. 파일을 대시보드에서 직접 실행했다면 `supabase migration repair --status applied <버전>` 으로 원장을 맞춥니다.
