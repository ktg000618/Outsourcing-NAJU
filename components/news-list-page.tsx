...
/** 1쪽은 /news, 그 뒤는 /news/page/2 … — 쿼리스트링이면 페이지가 동적이 되어 1시간 캐시를 잃는다. */
export const newsPageHref = (n: number) =>
  n <= 1 ? "/news" : `/news/page/${n}`;

/**
 * 소식 목록 한 쪽. /news 와 /news/page/[n] 이 같은 화면을 그린다.
 * 목록은 요약만 — 날짜·제목·첫 사진·두 줄. 본문과 사진 전부는 글 페이지에 있다.
 * 20건이 넘어가며 한 페이지에 사진 60장을 펼치던 것을 접었다. 10건씩 넘긴다.
 */
export async function NewsListPage({ page }: { page: number }) {
  const posts = await getPublishedPosts();
  const pageCount = Math.max(1, Math.ceil(posts.length / NEWS_PER_PAGE));
